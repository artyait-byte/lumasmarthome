...
- It assumes **limited cash and no debt pile-up**: Seed Cap plus Partner B's **narrow start envelope** (§7) — AI tools, website, aggregator lead-gen (Angi, Thumbtack, Houzz, Yelp), test Meta ads only after packages are ready, photo/video at the start, courses/certificates (e.g. SBDC, public-tender access). Hardware from client deposits. If the funnel is not converting, paid marketing is not scaled and the budget is not burned (§8). Он исходит из ограниченных денег и без накопления долгов: узкий стартовый конверт §7; если воронка не работает — основной маркетинг не двигаем и бюджет не сливаем.
